...
function getLang($default, $browserDetect = false)
{
    ...
    if ($getlang && file_exists("translations/".$getlang.".php")) {
        $lang = $getlang;
        $_SESSION['lang'] = $lang;
    }
...
function mergeImageSvg($back = false, $front = false)
{
...
function getBtcRates()
{
    ...
    $output .= '<small class="form-text text-muted">Обновление данных: '. date('d.m.Y', filemtime($cache_file)).'</small>';
